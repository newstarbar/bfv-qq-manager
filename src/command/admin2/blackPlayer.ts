...
export class BlackPlayerCommand implements Command {
    ...
    async execute(command: string, group_id: number, message_id: number, user_id: number): Promise<any> {
        if (this.regex.test(command)) {
            ...
            const { isSuccess, content } = await addLocalBlackList(playerName, reason, admin_name, user_id);
            if (isSuccess) {
                // 顺便Ban一下
                banPlayerCommand(playerName, `本地黑名单[${reason}]`, group_id, message_id, user_id);
