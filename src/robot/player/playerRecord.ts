import { AllVechileDate, AllWeaponDate, PlayerRecordDataCache, VechileData, WeaponData } from "../../interface/player";
...
export async function refreshPlayerData(
	...
	vehicles: AllVechileDate[],
	type: "join" | "settle" | "leave"
): Promise<{ sumKills: number; sumTime: number; sumWeaponKills: number; sumWeaponTime: number; sumVehicleKills: number; sumVehicleTime: number }> {
...
/** 保存玩家的一局数据 */
export async function savePlayerRecord(
	playerName: string,
	personaId: number,
	serverName: string,
	weaponKills: number,
	...
	const params = [
		...
		personaId,
		serverName,
		weaponKills + vehicleKills,
		weaponSeconds + vehicleSeconds,
		weaponKills,
		weaponSeconds,
		vehicleKills,
		vehicleSeconds,
		JSON.stringify(weapons),
		JSON.stringify(vechiles),
		today
	];
