...
/** 添加全局黑名单 */
export async function addGlobalBlackList(name: string, personaId: number, reason: string, group_name: string, group_id: number, admin_name: string, admin_qq: number, time: string): Promise<{ isSuccess: boolean, content: string }> {
    // 是否已经存在
