...
/** 获取群成员信息 */
export async function getMemberInfo(group_id: number | null, user_id: number | null, player_name: string | null): Promise<any[]> {
...
/** 新增或更新群成员信息 */
export async function addOrUpdateMemberInfo(group_id: number, user_id: number, player_name: string, admin_level: number, persona_id: number, join_time: number): Promise<void> {
	const db = new SQLiteDB(url, createTableSql);
	await db.open();
	// 查看该玩家是否存在
